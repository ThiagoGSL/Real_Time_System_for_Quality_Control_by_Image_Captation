...
def verify_objects(col_init, col_end):
    ...
    global times_verify_object
    
    client = RemoteAPIClient() #Requesting conection with API Client
    sim = client.require('sim')
    
    while True:
        #Setting inicial time ignoring the above code because it is supposed to run once at the start/restart of the system
        s_time_verify_object = time.time()
        if not emergency_stop:
            ...
            if change_check_sum >= 2:
                emergency_stop = True
                stop_event.set()
                e_time_verify_object = time.time()
                times_verify_object.append(e_time_verify_object-s_time_verify_object)
                break
...
def reset_button():
    ...
    while True:
        ...
        if command == "x":
            emergency_stop = True
            stop_event.set()
            pd.Series(data=times_verify_object, name="Times").to_csv("times_verify_object.csv",index=False)
            break
        else:   
